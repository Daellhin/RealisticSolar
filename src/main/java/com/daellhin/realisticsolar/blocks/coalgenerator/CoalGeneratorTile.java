	    	...
	    	if (counter > 0) {
	            ...
	            handler.ifPresent(h -> {
	            	...
	            	if (stack.getItem() == Items.COAL|| stack.getItem() == Items.COAL) {
	            		if (stack.getItem() == Items.COAL) { TimeMultiplier = 1; coalBlock = false;}
	            		else if (stack.getItem() == Items.COAL_BLOCK) { TimeMultiplier = 9; coalBlock = true;}
                    ...
                    if (te != null) {
                        boolean doContinue = te.getCapability(CapabilityEnergy.ENERGY, direction).map(handler -> {
                                    if (handler.canReceive()) {
                                        int received = handler.receiveEnergy(Math.min(capacity.get(), Config.COALGENERATOR_SEND.get()), false);
